...
- **What it costs at your size**, a new guide page, `docs/guide/sizing.md`. It prices a small, a medium and a
  large deployment with `estimateCost()`, term by term: illustrative workloads, labelled as such, with every dollar
  figure generated by `bench/sizing.cjs` from the shipped estimator and `pnpm bench:sizing:check` failing CI when
  the page and the estimator disagree. It says what moves the large deployment's bill, the pointer refresh and
