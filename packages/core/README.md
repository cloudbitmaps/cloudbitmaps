...
a bounded HOT RAM cache over STORAGE immutable `.crbm` objects, serverless chunk-skipping intersection, the segment
...
that enforces it), and **every storage driver** (S3 · GCS · Azure Blob, each hosting both the storage tier and
the registry, each on its own subpath with the backend SDK as an optional peer dependency).
