 ...
 * names (`ONE_REDIS_HA_CLUSTER`), not the Redis the data would need — so the published chart can
...
// The chart is one flat node against pay-per-use, so it compares with one cluster whatever the data size: the
// default profile sizes Redis to the data instead, which is the estimator's verdict, not this chart's line.
...
// which is more than that set needs, so the benchmarks page says so, and where the line would sit against it.
const sizedRef = estimateCost({ segments: [{ sizeBytes: atRestBytes }], workload: base });
const sizedCluster = sizedRef.redisBaseline.cluster;

const results = {
  note:
    'Generated by `pnpm bench` from the shipped estimateCost(), at the default rates against ONE_REDIS_HA_CLUSTER. ' +
    'Do not edit by hand.',
  pricing: P.name,
  ...
  referenceRedis: {
    monthlyUSD: round(sizedRef.redisBaseline.monthlyUSD, 2),
    cluster: `${sizedCluster.nodes} × ${sizedCluster.nodeType}`,
    readCrossoverPerSec: round(sizedRef.redisCrossover.readsPerSec, 2),
  },
};
...
const rows = [
  ...
  [
    'The Redis the 1.2 GiB set needs',
    `$${results.referenceRedis.monthlyUSD.toFixed(2)}/mo`,
    `${results.referenceRedis.cluster}, the estimator's default`,
    `the line would sit at ${results.referenceRedis.readCrossoverPerSec} reads/s`,
  ],
];
