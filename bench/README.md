...
| `sizing.cjs` | A small, a medium and a large deployment — illustrative workloads, labelled as such — priced by the **shipped** estimator (`estimateCost`) at the default pricing, so the page cannot quote a bill the library would not. | `pnpm bench:sizing` (builds first) | the tables in `docs/guide/sizing.md` (between `SIZING` markers) | yes — `pnpm bench:sizing:check` fails if the tables are not what the estimator prices |
