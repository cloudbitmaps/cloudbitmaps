...
7. **Storage-agnostic core.** `packages/core/src/core/` imports no cloud SDK and no driver impl — only the driver interfaces; the main entry stays SDK-free and core never imports a flavor package. Enforce with dependency lint rules in CI (`pnpm lint:arch`).
