...
**Direction (September 2026).** `1.0` centres on the **loaded store**: sets are computed upstream and `load`ed
as immutable generations, then `pin`ned, read and chunk-skipping-intersected from anywhere — one bucket, no
background process. The **live tier** (per-call `add`/`remove` over a warm store, its compaction daemon and
partition leases) is leaving this line: the lifecycle engine and the five non-AWS warm drivers are removed
(archived at the git tag `archive/live-warm-tier`; `0.9.x` stays on npm) and the DynamoDB warm tier with
compaction follows. Every roaring-based engine that needs freshness micro-batches into immutable segments rather
than mutating a stored bitmap per call; that is the shape this library builds. A future live tier, if there is
demand, would be immutable delta generations on the same bucket. Until that step lands, the sections below that
describe the warm tier and compaction describe code that still ships.
...
- **Crash-safe compaction, as a pass you schedule** — two-phase commit (lease → pin → reconcile orphaned generations →
  ...
  quarantine, and shardable, budgeted, urgency-ordered discovery. The shipped CLI is deliberately
  SDK-free and covers the local-filesystem backend; a cloud deployment calls `runCompactionCycle` from its own
  short handler. Crash-at-every-durable-step recovers to the oracle in tests. Run `runCompactionCycle` from a cron, a Lambda on a timer, or a `CronJob`; the bundled daemon was removed ahead of `1.0` (see the `CHANGELOG`).
...
| **Warm** (deltas, OCC) | DynamoDB · local filesystem · in-memory |
...
- **The warm and registry tiers are separate seams.** The registry's *cloud* implementations are DynamoDB
  and S3 only (plus local-filesystem and in-memory), so a non-AWS **cloud** deployment — say GCS cold — still needs an S3 or DynamoDB registry. Native registries on the
...
| --- | --- | --- |
| **Workload** | read-mostly; hot + cold, warm deltas compacted by a scheduled pass | write-heavy hot-spotting on a single chunk |
...
| **Backends** | S3 cold + DynamoDB warm/registry — the validated pair | the others (GCS, Azure Blob, and the S3 registry): conformance-passing and correctness-clean, but not envelope-validated |
...
- **Any feature that taxes the hot path** (`add` / `has` / `remove` / `count` / `intersect`) to speed up a
  rare operation. If it can't be pushed to wiring time, a scheduled pass, an admin call, or the docs, it doesn't
  ship.
