...
| `connect(url)` | **shipped** — one string wires both drivers, so bucket and prefix are stated once instead of twice. `s3://`/`gs://`/`az://`/`file://`/`memory://`, the scheme naming the protocol rather than a vendor so an S3-compatible store is the same scheme with an endpoint. A shortcut, not a second configuration surface: it returns the same `CloudRoaring` the constructors do |
