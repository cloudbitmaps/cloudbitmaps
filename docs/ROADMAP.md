...
5. **A curated public surface — ✅ Shipped.** `@cloudbitmaps/core`'s main entry went from **110 exports to
   82**: the due-index scheduler, `.crbm` construction, object-key layout, AEAD associated data, budget
   ...
   and are listed in [`MIGRATING.md`](../MIGRATING.md#6-core-exports-only-what-it-supports). An adversarial
   review then reverted three of them — `readRetentionPolicy`, `aadFor` and `checkBudget` — each a case where
   a public type or field could only be produced or consumed by the symbol that had been cut, which is the
   test worth applying to any surface reduction. The API reference guard now runs in both directions.
