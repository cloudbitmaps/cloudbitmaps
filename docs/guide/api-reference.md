...
| `gcOrphanGenerations(ref, { cold, registry }, { keep?, minAgeMs?, now? })` → `number[]` | delete superseded generations — everything below `currentGen` except the newest `keep` (default 1) as a grace window for in-flight readers. **`minAgeMs`** adds the time half of that window, and is the half that actually protects a reader: a burst of publishes walks a generation out of a count-based window while it is still being read, so nothing is collected until the pointer has been still for `minAgeMs`. Measured from the registry's `currentGenSince` (when `currentGen` last changed) — **not** the stored object's age, which reports a week-old file superseded one second ago as safe to delete. Requires `now` (epoch-ms) because core reads no clock; a row with no `currentGenSince` has an *unknown* age and collects nothing until the next publish stamps it. Never touches `currentGen` or anything above it, and deletes nothing while `currentGen` is `null`. **Exception:** on a `destroyed` segment every generation is garbage and all are collected. Nothing schedules this: the erasure rewrite calls it with `keep: 0`, the retention sweep calls it on tombstones, and a caller writing generations by hand collects on its own cadence. Returns the generations deleted |
