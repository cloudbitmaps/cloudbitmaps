...
| `gcOrphanGenerations(ref, { cold, registry }, { keep?, minAgeMs?, now? })` → `number[]` | delete superseded generations — everything below `currentGen` except the newest `keep` (default 1) as a grace window for in-flight readers. **`minAgeMs`** adds the time half of that window and is the half that protects a reader: a burst of publishes walks a generation out of a count-based window while it is still being read. Each generation is dated by the **registry**, which records every generation that was current and when it stopped being — the bucket cannot supply this, because an orphan from a crashed load is indistinguishable from a real successor there. Requires `now` (epoch-ms) because core reads no clock. A generation the row cannot date is **kept**; one the pointer skipped was never current and is collected without waiting. Never touches `currentGen` or anything above it, and deletes nothing while `currentGen` is `null`. **Exception:** on a `destroyed` segment every generation is garbage and all are collected. Nothing schedules this: the erasure rewrite calls it with `keep: 0`, the retention sweep calls it on tombstones, and a caller writing generations by hand collects on its own cadence. Returns the generations deleted |
...
`GenerationDeps` (`{ cold, registry }` — what `nextGeneration` / `gcOrphanGenerations` take) · `SupersededGeneration` (`{ gen, at }` — one entry of `RegistryRecord.supersededGens`: a generation that was current, and when it stopped being) · `EraseIdDeps` ·
