...
`storage` is the **only required option**; add `registry` for one-read generation resolution, encrypted segments,
...
your `registry` / `keystore`; or pass a pre-built `StorageChunkSource` (`MemoryStorageChunkSource`, or a
...
| `UnsupportedError` | (a) the bytes are well-formed but this build cannot read them — an unknown `.crbm` major version; or (b) this store's wiring cannot perform the operation, e.g. a lifecycle helper on a store built without a raw storage driver + registry | wire the store with what the operation needs, or upgrade the library | no |
...
`CloudRoaring` · `Segment` · `MemoryStorageDriver` · `MemoryRegistryDriver` · `MemoryStorageChunkSource` ·
...
`StorageChunkSource` · `SegmentRef` · `ChunkRef` · `GenKey` · `StorageCaps` · `RegCaps` · `RegistryRecord` ·
