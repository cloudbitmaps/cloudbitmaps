...
| `DEFAULT_MAX_CHECK_SEGMENTS` | default ceiling (250,000) on registry records one `checkConsistency` holds resident — raise via its `maxSegments` option |
...
`collectWithinBudget` · `DEFAULT_MAX_WARM_SCAN_BYTES` · `DEFAULT_MAX_CHECK_SEGMENTS` ·
