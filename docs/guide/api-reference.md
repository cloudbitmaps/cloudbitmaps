...
| `seg.intersectInto(dest, [other, …], opts?)` · `seg.unionInto(dest, [other, …], opts?)` · `seg.andNotInto(dest, [sup, …], opts?)` → `Promise<MaterializeResult>` | materialize the result as a **new generation of `dest`** — `dest`'s previous contents are superseded, not added to. Streaming, bounded memory, published forward-only, so readers of `dest` see the old generation or the new one, never a partial. **An empty result is refused, not published** — `dest` keeps what it had, and the result carries `published: false` with `reason: 'empty'`. Pass `allowEmpty: true` when emptying `dest` is the point, and `guard: { minCardinality, minRetained }` for the same plausibility bounds `load()` takes, judged against what `dest` held. A refusal is reported, not thrown — branch on `published`. A call involving an expired handle is refused earlier and harder (`ValidationError`). Throws `WriteConflictError` if a concurrent writer publishes a higher generation of `dest` first, rather than returning a generation that never became current. `opts.audit` emits `segment.publish` for the generation it lands. Needs a backend |
...
`EraseSubjectResult` · `MaterializeResult` · `BaseCombineOptions` · `CombineOptions` · `MaterializeOptions` · `EngineCombineOptions` ·
