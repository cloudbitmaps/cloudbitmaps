...
| `llms.txt` | A plain-text account of the library, written to be read and quoted by an AI assistant. Its version and its presence are gated. Its cost figures are not, unlike the pages', and nor is its count of "storage backends": the count check reads only a count of "drivers". |
