...
| `directory-readmes.test.ts` | Something has no row in its README — a file in `bench/` or `scripts/`; a directory, top-level file or gate here; a page, top-level file or `assets/` in `site/` — or a row names a path that does not exist, or one it cannot read. Only a row counts, not a mention, and tables in code blocks or comments are not rows. |
