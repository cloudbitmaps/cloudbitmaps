...
async function members(
  ...
  const source = new CrbmStorageChunkSource(storage, { registry, keystore });
  const store = new CloudRoaring({ storage: source, retry: false });
  const out: number[] = [];
  for await (const id of store.segment('s').iterate()) out.push(id);
  return out;
}

describe('CrbmStorageChunkSource — encryption end-to-end', () => {
  it('bulk-loads encrypted, stores the wrapped DEK, and reads back with the keystore', async () => {
    ...
    // The registry row carries the wrapped DEK; the storage object is encrypted (its footer hides cardinality).
    const rec = (await registry.get(SEG))!;
    expect(rec.wrappedDeks).toHaveLength(1);
    expect(rec.wrappedDeks![0]!.keyId).toBe('k1');

    expect(await members(storage, registry, keystore)).toEqual([...IDS].sort((a, b) => a - b));
  });
  ...
  it('reading an encrypted segment WITHOUT a keystore throws KeyUnavailableError', async () => {
    ...
    const keystore = new InProcessKeystore({ keys: { k1: k() }, activeKeyId: 'k1' });
    await bulkLoadCrbmGeneration(storage, { ...SEG, generation: 0 }, IDS, { registry, keystore });
    await expect(members(storage, registry /* no keystore */)).rejects.toBeInstanceOf(
      KeyUnavailableError,
    );
  });
  ...
  it('reading with a keystore that lacks the KEK throws KeyUnavailableError', async () => {
    const { storage, registry } = world();
    await bulkLoadCrbmGeneration(storage, { ...SEG, generation: 0 }, IDS, {
      registry,
    ...
    await expect(members(storage, registry, wrongKeystore)).rejects.toBeInstanceOf(
      KeyUnavailableError,
    );
  });
  ...
  it('reading with the right keyId but the wrong key bytes throws IntegrityError', async () => {
    ...
    await expect(members(storage, registry, imposter)).rejects.toBeInstanceOf(IntegrityError);
  });
  ...
  it('recovery KEK: a segment wrapped under active+recovery reads with the recovery key alone', async () => {
    ...
    expect(await members(storage, registry, recoveryOnly)).toEqual([...IDS].sort((a, b) => a - b));
  });

  it('bulk-load encryption requires a registry to store the wrapped DEK', async () => {
    const { storage } = world();
    await expect(
      bulkLoadCrbmGeneration(storage, { ...SEG, generation: 0 }, IDS, {
        keystore: new InProcessKeystore({ keys: { k1: k() }, activeKeyId: 'k1' }),
      }),
  ...
  it('constructing a source with a keystore but no registry fails fast', () => {
    expect(
      () =>
        new CrbmStorageChunkSource(new MemoryStorageDriver(), {
          keystore: new InProcessKeystore({ keys: { k1: k() }, activeKeyId: 'k1' }),
        }),
  ...
  it('cleartext still works with no keystore (encryption is opt-in)', async () => {
    const { storage, registry } = world();
    await bulkLoadCrbmGeneration(storage, { ...SEG, generation: 0 }, IDS, { registry }); // no keystore
    expect((await registry.get(SEG))!.wrappedDeks).toBeUndefined();
    expect(await members(storage, registry)).toEqual([...IDS].sort((a, b) => a - b));
  });
});

// The Memory drivers clone object references; this proves the wrapped-DEK list survives a real JSON
// serialize↔parse round-trip through the persistent LocalFs storage + registry drivers (the realistic failure mode).
describe('CrbmStorageChunkSource — encryption over persistent (LocalFs) drivers', () => {
  let root: string;
  let n = 0;
  beforeAll(async () => {
