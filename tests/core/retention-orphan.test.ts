 ...
 * nothing and by one whose every `storage.delete` threw** — and only the second leaves data behind.
 *
...
/** A storage driver whose deletes always fail — a 403, a bucket policy, a throttle. */
function undeletable(real: MemoryStorageDriver): IStorageDriver {
  return {
...
describe('retireExpired distinguishes "deleted nothing" from "there was nothing"', () => {
  it('keeps the row when the objects survive, so the data stays reclaimable', async () => {
    ...
    const res = await retireExpired({ storage: undeletable(storage), registry }, { now: PAST + 1 });
    ...
    const second = await retireExpired({ storage, registry }, { now: PAST + 48 * 3_600_000 });
  ...
  it('still purges the row for a name that genuinely held nothing', async () => {
    ...
    const res = await retireExpired({ storage, registry }, { now: PAST + 1 });
  ...
  it('the data is NOT stranded: everything that could reclaim it still can', async () => {
    ...
    await bulkLoadCrbmGeneration(storage, { ...REF, generation: 0 }, [1, 2, 3], { registry });
    await setSegmentRetention(REF, { registry }, { expiresAt: PAST });
    await retireExpired({ storage: undeletable(storage), registry }, { now: PAST + 1 });

    // A tombstoned row means EVERY generation is collectable — the row is what makes that true.
    expect(await gcOrphanGenerations(REF, { storage, registry })).toEqual([0]);
    expect(await generations(storage)).toEqual([]);
  });
  ...
  it('the expired object is genuinely still readable while it survives — this is the harm', async () => {
    ...
    const reader = await openGenerationReader(storage, { ...REF, generation: 0 }, undefined);
