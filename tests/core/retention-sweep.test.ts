 ...
 *  - **Purging a tombstone cannot strand storage.** Deleting the row makes the segment invisible to
 *    `gcOrphanGenerations`, and nothing else would ever collect a tombstoned segment's objects — so the storage
 *    must be provably gone before the row is.
 */
...
function faultyCold(base: IStorageDriver, overrides: Partial<IStorageDriver>): IStorageDriver {
  return {
...
describe('retireExpired — selection', () => {
  it('retires the expired segments and leaves everything else alone', async () => {
    const w = world();
    const store = w.store();
    // An expired segment on its first generation, an expired one that has been re-loaded, one not due yet, and
    // one with no policy at all.
    await w.load('gone-once', [1, 2, 100_000]);
    await store.setRetention({ segment: 'gone-once' }, { expiresAt: EXPIRED });

    await w.load('gone-twice', [3, 4]);
    await w.load('gone-twice', [3, 4, 5]);
    await store.setRetention({ segment: 'gone-twice' }, { expiresAt: EXPIRED });

    await w.load('later', [5]);
    await store.setRetention({ segment: 'later' }, { expiresAt: FUTURE });

    await w.load('forever', [6]);
    ...
    expect(entries.get('gone-once')).toMatchObject({ action: 'retired', expiresAt: EXPIRED });
    expect(entries.get('gone-twice')).toMatchObject({ action: 'retired', expiresAt: EXPIRED });
    ...
    expect(await coldGenerations(w.storage, { segment: 'gone-once' })).toEqual([]);
    expect(await coldGenerations(w.storage, { segment: 'gone-twice' })).toEqual([]);
    expect(await coldGenerations(w.storage, { segment: 'later' })).toEqual([0]);
  ...
  it('reports a malformed policy instead of silently treating it as "never expires"', async () => {
    ...
    await w.load('bad', [1]); // the load publishes gen 0 onto the row, policy and all
  ...
  it('scopes to one namespace', async () => {
    ...
    for (const namespace of ['a', 'b']) {
      await w.load({ namespace, segment: 'day' }, [1]);
...
describe('retireExpired — the guards that make it safe to point at a fleet', () => {
  it('dryRun reports what it would retire and deletes nothing', async () => {
    ...
    // The preview goes through `dropSegment` itself, so the numbers are the real ones, not a guess — and it
    // surfaces the irreversible half (`wouldCryptoShred`), which a preview of the recoverable half only would not.
    ...
    expect(await coldGenerations(w.storage, { segment: 'd' })).toEqual([0]);
  ...
  it('isolates a per-segment fault into the ledger and keeps sweeping', async () => {
    ...
    const storage = faultyCold(w.storage, {
    ...
    expect(await coldGenerations(w.storage, { segment: 'boom' })).toEqual([0]);
...
describe('retireExpired — what the adversarial review found', () => {
  it('bounds destruction when every drop faults — the limit must charge ATTEMPTS', async () => {
    ...
    const storage = faultyCold(w.storage, { list: () => listUnavailable() });
  ...
  it('never purges a GDPR crypto-shred tombstone, even one that carried a policy', async () => {
    ...
    await destroySegment({ segment: 'gdpr' }, { registry: w.registry }, { confirmSegment: 'gdpr' });
  ...
  it('does not brick a name whose segment never had any data', async () => {
    ...
    // And the name is loadable again, rather than refused forever by a fence for data that never existed.
    await w.load('typo', [1]);
...
describe('retireExpired — tombstone purge', () => {
  ...
  it('collects a straggler Storage generation itself instead of reporting it forever', async () => {
    ...
    expect(await coldGenerations(w.storage, { segment: 'day' })).toEqual([]); // the billing leak is gone
    expect(await w.registry.get({ segment: 'day' })).toBeNull();
  });
  ...
  it('keeps the row when the straggler survives the GC — the storage is still out there', async () => {
    // The other half of the self-heal: the tier answers, but the delete does not land (a denied lifecycle policy,
    // an object-lock retention window). The GC is best-effort, so the sweep must re-check rather than assume, and
    // report `tombstone-not-empty` rather than purge a row that is the only thing pointing at those bytes.
    const w = world();
    await w.load('day', [1]);
    await w.store().setRetention({ segment: 'day' }, { expiresAt: EXPIRED });
    await retireExpired(w.dropDeps, { now: T0 });
    ...
    expect((await w.registry.get({ segment: 'day' }))!.status).toBe('destroyed');
    expect(await coldGenerations(w.storage, { segment: 'day' })).toEqual([7]); // still there, still reported
  });
...
describe('store.retireExpired', () => {
  ...
  it('retires every generation of a re-loaded segment, not just the current one', async () => {
    // A re-load leaves the superseded object in the bucket until something collects it, so "retired" has to mean
    // the whole segment is gone — a drop that reclaimed only `currentGen` would leave readable bytes behind a
    // tombstone and keep billing for them.
    const w = world();
    await w.load('day', [1, 2, 100_000]);
    await w.load('day', [1, 2, 3, 100_000]);
    await w.store().setRetention({ segment: 'day' }, { expiresAt: EXPIRED });
    expect(await coldGenerations(w.storage, { segment: 'day' })).toEqual([0, 1]);
    ...
    expect(await coldGenerations(w.storage, { segment: 'day' })).toEqual([]);
