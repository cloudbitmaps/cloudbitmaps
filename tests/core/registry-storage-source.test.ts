...
let n = 0;
beforeAll(async () => {
  root = await mkdtemp(join(tmpdir(), 'crbm-reg-storage-'));
});
afterAll(async () => {
...
describe('registry-aware CrbmStorageChunkSource', () => {
  ...
  it('a registry-aware source returns empty when there is no registry row', async () => {
    ...
    const source = new CrbmStorageChunkSource(storage, { registry: new MemoryRegistryDriver() });
  ...
  it('coalesces a concurrent burst at the TTL boundary into ONE registry re-resolve', async () => {
    ...
    await bulkLoadCrbmGeneration(storage, { ...SEG, generation: 0 }, [1, 2, 3], { registry: base });
