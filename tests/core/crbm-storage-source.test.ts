...
describe('CrbmStorageChunkSource + writeCrbmGeneration', () => {
  it('run-encodes a storage generation: far smaller on disk, byte-identical set on the way back', async () => {
    ...
    const chunk = await storage.getChunk({ segment: 'runs', chunkKey: 0 });
  ...
  it('leaves a sparse generation byte-identical — the pass is never a losing trade', async () => {
    ...
    const storage = new CrbmStorageChunkSource(driver);
    const chunk = await storage.getChunk({ segment: 'sparse', chunkKey: 0 });
  ...
  it('round-trips bitmaps through an on-disk generation', async () => {
    ...
    expect((await storage.listChunkKeys({ segment: 's' })).sort((a, b) => a - b)).toEqual([0, 5]);
    ...
    expect(await storage.getChunk({ segment: 's', chunkKey: 999 })).toBeNull();
  });
  ...
  it('skips empty bitmaps (empty chunks are never stored)', async () => {
    ...
    const storage = new CrbmStorageChunkSource(driver);
    expect(await storage.listChunkKeys({ segment: 's' })).toEqual([0]);
  });
  ...
  it('returns empty/null for a segment with no generations', async () => {
    const storage = new CrbmStorageChunkSource(driver);
    expect(await storage.listChunkKeys({ segment: 'ghost' })).toEqual([]);
    expect(await storage.getChunk({ segment: 'ghost', chunkKey: 0 })).toBeNull();
  });

  const read = async (storage: CrbmStorageChunkSource, chunkKey: number): Promise<number[]> =>
    SafeBitmap.safeDeserialize(
      (await storage.getChunk({ segment: 's', chunkKey }))!,
      1 << 20,
    ).toArray();
  ...
  it('resolves the latest generation across gaps and pins it per source', async () => {
    ...
    const storage = new CrbmStorageChunkSource(driver);
    expect(await read(storage, 0)).toEqual([9]); // latest = gen 5, not 1
    ...
    expect(await read(storage, 0)).toEqual([9]);
    // …but a fresh source picks it up.
    expect(await read(new CrbmStorageChunkSource(driver), 0)).toEqual([9, 10]);
  });
  ...
  it('surfaces a corrupt on-disk .crbm as IntegrityError', async () => {
    ...
    const storage = new CrbmStorageChunkSource(driver);
    await expect(storage.getChunk({ segment: 's', chunkKey: 0 })).rejects.toBeInstanceOf(
      IntegrityError,
    );
  });
  ...
  it('does not cache a failed open — a later read retries', async () => {
    ...
    await expect(storage.getChunk({ segment: 's', chunkKey: 0 })).rejects.toThrow('transient');
    expect(await read(storage, 0)).toEqual([1]); // retried, not poisoned-cached
  });

  it('fails fast if the storage driver lacks range reads', () => {
    const noRange = {
