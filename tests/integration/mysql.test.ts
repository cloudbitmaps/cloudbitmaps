// Runs against the MySQL service from docker-compose (see docker-compose.yml): `docker compose up -d`
// then `pnpm test:integration`. A real MySQL (also covers MariaDB — same SQL + OCC), not an emulator — the
// OCC is plain SQL (INSERT + token-fenced UPDATE/DELETE, affectedRows-checked).
import { createPool, type Pool } from 'mysql2/promise';
import { warmConformance } from '@/testing/conformance';
import { MysqlWarmDriver, mysqlWarmTableDDL } from '@/drivers/mysql/warm';
...
const pool: Pool = createPool({
  ...
  port: PORT,
  user: 'cloudroaring',
  password: 'cloudroaring',
  database: 'cloudroaring',
  connectionLimit: 8,
});

beforeAll(async () => {
  // `docker compose up --wait` returns when healthy, but poll anyway so a cold start (MySQL's first-boot init
  // can be slow) can't red the suite.
  for (let attempt = 0; ; attempt++) {
    try {
      await pool.query('SELECT 1');
      break;
    } catch (err) {
      if (attempt >= 60) throw err; // ~60s
      await new Promise((r) => setTimeout(r, 1000));
    }
  }
  await pool.query(mysqlWarmTableDDL(TABLE));
...
const freshDriver = (): IWarmDriver =>
  new MysqlWarmDriver({ pool, table: TABLE, keyPrefix: `conf-${n++}` });

// The MySQL driver must pass the SAME warm contract as in-memory + LocalFs + DynamoDB + Postgres + Redis +
// Mongo + Cassandra (finding V8) — against real, cross-process SQL OCC.
warmConformance('MysqlWarmDriver (mysql:8)', freshDriver);

describe('MysqlWarmDriver specifics (mysql:8)', () => {
  const ref = (chunkKey: number) => ({ segment: 's', chunkKey });

  it('isolates logical stores by keyPrefix (same table, no cross-talk)', async () => {
    const a = new MysqlWarmDriver({ pool, table: TABLE, keyPrefix: `iso-a-${n}` });
    const b = new MysqlWarmDriver({ pool, table: TABLE, keyPrefix: `iso-b-${n}` });
    n++;
  ...
  it('matches segment/keyPrefix case-sensitively (utf8mb4_bin, not the default ci collation)', async () => {
    // MySQL's default collation is case-insensitive; the driver's table pins utf8mb4_bin. If it regressed to a
    // ci collation, the lowercase driver below would see the uppercase row and this would fail.
    const upper = new MysqlWarmDriver({ pool, table: TABLE, keyPrefix: `Case-${n}` });
    const lower = new MysqlWarmDriver({ pool, table: TABLE, keyPrefix: `case-${n}` });
    n++;
    await upper.putConditional(ref(0), Uint8Array.of(7), NO_ROW);
    expect(await lower.get(ref(0))).toBeNull(); // 'case-' must NOT match 'Case-'
    // ...and a case-differing SEGMENT is likewise distinct within one prefix.
    const d = new MysqlWarmDriver({ pool, table: TABLE, keyPrefix: `seg-${n++}` });
    await d.putConditional({ segment: 'Seg', chunkKey: 0 }, Uint8Array.of(1), NO_ROW);
    expect(await d.get({ segment: 'seg', chunkKey: 0 })).toBeNull();
  });
  ...
  it('mysqlWarmTableDDL is idempotent (safe to run on every deploy)', async () => {
    await expect(pool.query(mysqlWarmTableDDL(TABLE))).resolves.toBeDefined();
    await expect(pool.query(mysqlWarmTableDDL(TABLE))).resolves.toBeDefined();
  });
});

describe('MysqlWarmDriver end-to-end through the engine (mysql:8)', () => {
  ...
  it('tier-merges a cold base with live warm add/remove + chunk-skipping intersect', async () => {
    ...
    const store = new CloudRoaring({
      warm: new MysqlWarmDriver({ pool, table: TABLE, keyPrefix: `e2e-${n++}` }),
  ...
  it('lists a wide segment completely and in ascending order at the default page size', async () => {
    ...
    const d = new MysqlWarmDriver({ pool, table: TABLE, keyPrefix: `scale-${n++}` });
    const N = 2100;
